...
class ImageEmbedinator(Embedinator):

    def __init__(self, bot, destination, member=None, **kwargs):
        super().__init__(bot, destination, member, **kwargs)
        self.bot = bot
        self.destination = destination
        self.member = member
        self.base_embed = discord.Embed(**kwargs)
    ...
    def add_image(self, url):
        if not self.last_page:
            self.add_embed_page()
        elif self.last_page.image != discord.Embed.Empty:
            self.add_embed_page()
        self.last_page.set_image(url=url)
